...
%add that the performance is mainly driven by agent density rather than topology inference scheme

 %mention small impact of final fitness value, but take longer to reach the local minimun for the same given exerimental time, this suggests that the tradeoff for lower latency/jitter is the time it takes to reach the same value in ters of fitness values. Local pockets with different information happen more oftern in COMM AWARE scheme
